...
6. Performance check: Open DevTools Performance tab, record page load, verify:
   - Math editor loads in under 500ms (from navigation to interactive)
   - No long tasks blocking main thread during symbol insertion
</verification>
...
6. Page load to interactive state completes in under 500ms (measured via DevTools Performance tab)
</success_criteria>
