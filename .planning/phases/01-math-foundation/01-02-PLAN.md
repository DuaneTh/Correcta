...
must_haves:
  truths:
    ...
    - "Math displays identically in student answer review (ResultsView)"
  ...
  key_links:
    ...
    - from: "app/student/attempts/[attemptId]/results/ResultsView.tsx"
      to: "components/exams/MathRenderer.tsx"
      via: "imports MathRenderer for question/answer/feedback display"
      pattern: "import MathRenderer from"
    - from: "app/dashboard/exams/[examId]/grading/[attemptId]/GradingView.tsx"
      to: "components/exams/MathRenderer.tsx"
      via: "should use MathRenderer for question/answer display (currently plain text)"
      pattern: "import MathRenderer"
---

<objective>
Replace MathJax with KaTeX for consistent, fast math rendering across all display surfaces.

Purpose: Research identified KaTeX as the standard choice (10x faster than MathJax, consistent rendering). Current MathRenderer uses MathJax which causes rendering inconsistencies with MathLive editor. MATH-05 requires visual parity across editor and web display surfaces (editor preview, student answer review, teacher grading view). PDF export parity is addressed separately in Phase 5 (EXPO-04).

Output: KaTeXRenderer component and updated MathRenderer using KaTeX. All math displays consistently regardless of where it appears in the web application.

Note: Phase 1 scope for MATH-05 covers editor and web display surfaces only. PDF rendering consistency will be verified in Phase 5 using @react-pdf/renderer with the renderLatexToString export created here.
</objective>
...
# Answer review surfaces that use MathRenderer
...
2. Answer review surface test:
   - Create an exam with math questions, submit as student
   - Navigate to student results page (/student/attempts/[id]/results)
   - Verify math in question content, student answer, and feedback all render correctly
   - Navigate to teacher grading page (/dashboard/exams/[id]/grading/[attemptId])
   - Verify math displays correctly (note: GradingView may need MathRenderer added if currently plain text)
...
<success_criteria>
1. Equation entered in MathLive displays identically in editor preview and answer review surfaces (MATH-05 web scope)
...
6. Student ResultsView renders math correctly for questions, answers, and feedback
7. Teacher GradingView renders math correctly (add MathRenderer import if missing)

Note: PDF export visual parity (MATH-05 PDF scope) will be verified in Phase 5 (EXPO-04).
</success_criteria>
