...
3. Create `components/exams/WebcamCapture.tsx`:
   ...
   - useEffect with setInterval at `intervalSeconds * 1000`:
     ...
     e. Fire-and-forget (catch errors silently, don't block exam)
   - Also log a WEBCAM_SNAPSHOT event to proctor-events API on each capture (for the timeline)
  ...
  <done>
    CameraPermissionCheck tests and verifies camera access with clear UX. WebcamCapture shows small preview and captures periodic snapshots, uploading to the snapshot API.
  </done>
</task>
...
CRITICAL: Do not break existing exam start or exam room flow. All proctoring additions are conditional on config.
  ...
  <done>
    Snapshot upload API stores to MinIO and enqueues analysis job. ExamStartPage shows camera check and consent when webcam proctoring enabled. ExamRoomClient shows webcam preview and captures snapshots during exam.
  </done>
</task>
...
5. Camera permission check works before exam start
6. Webcam preview visible during proctored exam
7. Snapshots captured at configured interval
8. No impact when proctoring disabled
</verification>
