...
<task type="auto">
  ...
  <action>
0. FIRST: Read the full exam editor page (`app/teacher/exams/[examId]/edit/page.tsx`) to detect the form state management pattern. Determine which pattern is used:
   - React Hook Form (look for `useForm`, `register`, `handleSubmit`, `setValue`)
   - Zustand store (look for `useExamStore`, `create(...)`)
   - useState (look for `useState`, `setFormData`)
   - Server actions (look for `useFormState`, `formAction`)
   Note the exact pattern, variable names, and how existing fields are read/written. ProctoringConfigPanel MUST integrate with this same pattern — do NOT introduce a separate state mechanism.
...
2. Integrate into exam editor page using the SAME form state pattern detected in step 0:
   - If React Hook Form: use `setValue('antiCheatConfig', ...)` and `watch('antiCheatConfig')` or register a custom field
   - If Zustand: use the store setter to update antiCheatConfig
   - If useState: add antiCheatConfig to the state object and update via setFormData
...
IMPORTANT: Do NOT introduce a separate useState for proctoring config if the form uses React Hook Form or Zustand. Follow the detected pattern exactly.
  </action>
  <verify>
    Run `npx tsc --noEmit` to verify no type errors.
    Run `npm run build` to verify the page compiles.
    Grep for the form state integration pattern in exam editor (e.g., "antiCheatConfig" appears in the form save logic).
    Visually: The exam editor should show a collapsible "Proctoring" section with toggles.
  </verify>
  <done>
    ProctoringConfigPanel renders toggles for webcam, lockdown, interval, sensitivity. It is integrated into the exam editor using the same form state pattern as existing fields, and saves to antiCheatConfig JSON field. Teacher can configure proctoring per exam.
  </done>
</task>
...
7. ProctoringConfigPanel uses the same form state pattern as the rest of the exam editor
</verification>
...
- Panel integrates with existing form state management (not a separate state)
</success_criteria>
