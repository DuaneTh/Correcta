...
<task type="auto">
  ...
  <action>
    ...
    - `onDragEnd` converts pixel position back to graph coords using pixelToGraph and calls onUpdate
    - When grid is enabled (axes.showGrid), snap drag results to grid using snapToGrid from coordinate-utils
    ...
    CRITICAL: Check if sampleFunction, evaluateExpression, and convertLatexToExpression are exported from graph-utils.ts. If they are not exported (only used internally), you need to add `export` to those functions in graph-utils.ts. The minimum needed exports are the expression evaluation pipeline so EditableFunction can sample points. Add graph-utils.ts to files_modified if exports are added.
  </action>
  <verify>
    - `npx tsc --noEmit` passes
    ...
    - Manual drag test: In a temporary test page or storybook, render GraphCanvas with a single point at (0,0) on axes -5..5. Drag the point to approximately (2, 3). Verify the onUpdate callback receives graph coordinates close to (2, 3) -- confirming Y-axis inversion works correctly and pixelToGraph produces correct values.
    - Grid snap test: With axes.showGrid=true and gridStep=1, drag a point. Verify resulting coordinates are snapped to integer values (e.g., dragging near (1.8, 2.3) produces (2, 2)).
  </verify>
  <done>
    All 6 graph element types have interactive Konva shape components. Points and text are draggable. Line/curve endpoints are draggable. Functions have domain handles. Areas render filled regions. Coordinate conversion verified via drag test with Y-axis inversion. Grid snapping confirmed working.
  </done>
</task>
...
- Grid snapping works when axes.showGrid is enabled
