...
must_haves:
  truths:
    ...
    - "Clicking a shape in the palette adds it to the canvas with sensible defaults"
  ...
  key_links:
    ...
    - from: "components/exams/graph-editor/SimpleGraphEditor.tsx"
      ...
      via: "Renders GraphCanvas for interactive editing"
...
Purpose: Delivers the core user experience -- teachers can add shapes from a palette to an interactive canvas (Simple mode) or use the form-based editor (Advanced mode), with seamless switching between modes.
...
<task type="auto">
  ...
  <action>
    ...
    2. Create `components/exams/graph-editor/SimpleGraphEditor.tsx`:
       ...
       - Shape insertion: when ShapePalette calls onAddShape, call template.createElements(graph.axes) to get new elements, merge them into the current graph payload arrays (spread existing + new), call onChange
    ...
    IMPORTANT: Do NOT use dnd-kit for palette-to-canvas drag. The research suggested it but click-to-add is simpler, more reliable, and sufficient for v1. Shapes are added at center of current viewport / default position. Drag-and-drop from palette can be added later. Keep it simple.
  </action>
  <verify>
    - `npx tsc --noEmit` passes
    - ShapePalette renders categorized shape items
    - SimpleGraphEditor shows palette on left, canvas on right
    - Clicking a shape template adds it to the graph (new element appears in GraphPayload)
    - Selected element shows properties bar below canvas
  </verify>
  <done>
    Simple mode editor complete: shape palette, interactive canvas, properties bar, axes config panel.
  </done>
</task>
...
- Clicking a shape in Simple mode adds it to the canvas
...
- Shape palette provides quick insertion of common math shapes
