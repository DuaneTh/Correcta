...
2. **Get selected area from state** (add after selected state declaration):
   ```typescript
   // Get the selected area object from value.areas using selectedId
   const selectedArea = useMemo(() => {
     if (selected?.type !== 'area') return null
     return value.areas.find(a => a.id === selected.id) || null
   }, [selected, value.areas])
   ```

3. **Add area update handler:**
   ```typescript
...
4. **Modify the mini properties bar section** (currently ~line 371-391):
   ...
   ```typescript
   {/* Properties bar - conditional rendering based on selection type */}
   {selected && (
     <div className="...existing styles...">
       {selected.type === 'area' && selectedArea ? (
         ...
         // Keep existing mini properties bar for points, lines, functions
         <div className="...existing content...">
           {/* existing display for non-area selections */}
         </div>
       )}
     </div>
   )}
